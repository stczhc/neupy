...
! *****************************************************************************
    ...
    INTEGER, INTENT(in)                      :: n, m
    REAL(8), INTENT(inout)             :: x(n)
    REAL(8), INTENT(in)                :: lower_bound(n), upper_bound(n)
    INTEGER                                  :: nbd(n)
    REAL(8) :: f, g(n), factr, pgtol, ws(n, m), wy(n, m), sy(m, m), &
    ...
    REAL(8) :: cachyt, cpu1, cpu2, ddot, ddum, dnorm, dr, dtd, epsmch, &
...
! *****************************************************************************
    ...
    INTEGER                                  :: m
    REAL(8)                            :: sy(m, m), wt(m, m)
    INTEGER                                  :: col
    REAL(8), INTENT(in)                :: v(2*col)
    REAL(8), INTENT(out)               :: p(2*col)
...
! *****************************************************************************
    ...
    INTEGER, INTENT(in)                      :: n
    REAL(8), INTENT(in)                :: x(n), lower_bound(n), &
                                                upper_bound(n)
    INTEGER, INTENT(in)                      :: nbd(n)
    REAL(8), INTENT(in)                :: g(n)
    INTEGER                                  :: iorder(n)
    INTEGER, INTENT(inout)                   :: iwhere(n)
    REAL(8)                            :: t(n), d(n), xcp(n)
    INTEGER, INTENT(in)                      :: m
    REAL(8), INTENT(in)                :: sy(m, m), wt(m, m), theta
    INTEGER, INTENT(in)                      :: col
    REAL(8), INTENT(in)                :: ws(n, col), wy(n, col)
    INTEGER, INTENT(in)                      :: head
    REAL(8)                            :: p(2*m), c(2*m), wbp(2*m), &
                                                v(2*m)
    INTEGER                                  :: nseg, iprint
    REAL(8), INTENT(in)                :: g_inf_norm
    ...
    REAL(8)                            :: bkmin, ddot, dibp, dibp2, dt, &
...
!           (if iter=2, initialize heap).
      ...
      IF (ABS(dt) > 1D-15 .AND. iprint >= 100) THEN
...
! *****************************************************************************
    ...
    INTEGER, INTENT(in)                      :: n, m
    REAL(8), INTENT(in)                :: x(n), g(n), ws(n, m), &
                                                wy(n, m), sy(m, m), wt(m, m), &
                                                z(n)
    REAL(8), INTENT(out)               :: r(n), wa(4*m)
...
! *****************************************************************************
    ...
    INTEGER, INTENT(in)                      :: n, m
    REAL(8), INTENT(in)                :: factr, lower_bound(n), &
                                                upper_bound(n)
    INTEGER                                  :: nbd(n)
    CHARACTER(LEN=60)                        :: task
    INTEGER                                  :: info, k
...
! *****************************************************************************
    ...
    INTEGER, INTENT(in)                      :: m
    REAL(8)                            :: wn1(2*m, 2*m)
    REAL(8), INTENT(out)               :: wn(2*m, 2*m)
    REAL(8), INTENT(in)                :: ws(n, m), wy(n, m), sy(m, m), &
                                                theta
...
! *****************************************************************************
    ...
    REAL(8), PARAMETER :: big = 1.0D10, ftol = 1.0D-3, &
      gtol = 0.9D0, one = 1.0D0, xtol = 0.1D0, zero = 0.0D0
...
! *****************************************************************************
    ...
    INTEGER                                  :: n, m
    REAL(8)                            :: ws(n, m), wy(n, m), sy(m, m), &
                                                ss(m, m), d(n), r(n)
    INTEGER                                  :: itail, iupdat, col, head
    REAL(8)                            :: theta, rr, dr, stp, dtd
...
! *****************************************************************************
    ...
    INTEGER, INTENT(in)                      :: n, m
    REAL(8), INTENT(in)                :: lower_bound(n), &
                                                upper_bound(n), x(n)
...
! *****************************************************************************
    ...
    REAL(8), INTENT(in)                :: lower_bound(n), upper_bound(n)
    INTEGER, INTENT(in)                      :: nbd(n)
    REAL(8), INTENT(inout)             :: x(n), d(n)
    REAL(8)                            :: xp(n)
    REAL(8), INTENT(in)                :: ws(n, m), wy(n, m), theta, &
                                                xx(n), gg(n)
    INTEGER, INTENT(in)                      :: col, head
    INTEGER, INTENT(out)                     :: iword
    REAL(8)                            :: wv(2*m)
    REAL(8), INTENT(in)                :: wn(2*m, 2*m)
    INTEGER                                  :: iprint
    INTEGER, INTENT(out)                     :: info
...
!
            ...
            IF ( nbd(k) == 1 ) THEN
               ...
               IF ( ABS(x(k) - lower_bound(k)) < 1D-15 ) iword = 1
            ELSE
!
               ...
               IF ( nbd(k) == 2 ) THEN
                  ...
                  IF ( ABS(x(k) - lower_bound(k)) < 1D-15 .OR. ABS(x(k) - upper_bound(k)) < 1D-15 ) iword = 1
               ELSE
!
                                             ! upper bounds only
                  IF ( nbd(k) == 3 ) THEN
                     x(k) = MIN( upper_bound(k), xk + dk )
                     IF ( ABS(x(k) - upper_bound(k)) < 1D-15 ) iword = 1
                  END IF
               END IF
            END IF
!
...
! *****************************************************************************
      SUBROUTINE dcsrch(f,g,stp,ftol,gtol,xtol,stpmin,stpmax,           &
                        task,isave,dsave)
    REAL(8)                            :: f, g
    REAL(8), INTENT(inout)             :: stp
    REAL(8)                            :: ftol, gtol, xtol, stpmin, &
                                                stpmax
    CHARACTER(LEN=*)                         :: task
    INTEGER                                  :: isave(2)
    REAL(8)                            :: dsave(13)

    REAL(8), PARAMETER                 :: p5 = 0.5D0, p66 = 0.66D0, &
                                                xtrapl = 1.1D0, &
                                                xtrapu = 4.0D0, zero = 0.0D0

    INTEGER                                  :: stage
    LOGICAL                                  :: brackt
    REAL(8)                            :: finit, fm, ftest, fx, fxm, &
...
!        Test for warnings.
         ...
         IF (ABS(stp - stpmax) < 1D-15 .AND. f <= ftest .AND. g <= gtest)        &
            task = 'WARNING: STP = STPMAX'
         IF (ABS(stp - stpmin) < 1D-15 .AND. (f > ftest .OR. g >= gtest))       &
...
! *****************************************************************************
      SUBROUTINE dcstep(stx,fx,dx,sty,fy,dy,stp,fp,dp_loc,brackt,           &
                        stpmin,stpmax)
    REAL(8), INTENT(inout)             :: stx, fx, dx, sty, fy, dy, stp
    REAL(8), INTENT(in)                :: fp, dp_loc
    LOGICAL, INTENT(inout)                   :: brackt
    REAL(8), INTENT(in)                :: stpmin, stpmax

    REAL(8), PARAMETER                 :: p66 = 0.66D0, &
                                                three = 3.0D0, two = 2.0D0, &
                                                zero = 0.0D0

    REAL(8)                            :: gamma, p, q, r, s, sgnd, &
...
!        to infinity in the direction of the step.
         ...
         r = p/q
         IF (r < zero .AND. ABS(gamma) >= 1D-15) THEN
            stpc = stp + r*(stx - stp)
...
!
         ...
         DO info = 1, n
!     ......exit
            IF (ABS(t(info,info)) < 1D-15) RETURN
         END DO
         info = 0
!
...
! *****************************************************************************
    ...
    REAL(8), INTENT(in) :: theta, fold, tol, dnorm, epsmch, cpu1, &
      cachyt, sbtime, lnscht, time1, gd, step_max, g_inf_norm, stp, gdold, dtd
