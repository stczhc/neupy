...
class LBFGS(object):
  ...
  def __init__(self, n):
    ...
    self.wa = np.zeros((lenwa, ), dtype=np.float64)
  ...
  def _step(self):
    ...
    while True:
      ...
      if self.stat == 1:
        self.f = np.array(self.p.eval(self.x), dtype=np.float64)
        self.g = np.array(self.p.evald(self.x), dtype=np.float64)
        self._invoke()
      elif self.stat >= 3:
        break
      just_entered = False
  ...
  def opt(self):
    ...
    if self.log_file != 0:
      print ('   ITER.   ENERGY(OLD)    ENERGY(NEW)      DE' + 
        ...
        '     STEPRMS')
    xold = np.array(self.x, dtype=np.float64)
    self.fold = np.array(self.p.eval(self.x), dtype=np.float64)
    ...
    while True:
      ...
      if self.stat == 4 or self.stat == 5:
        break
      xold = np.array(self.x, dtype=np.float64)
      self.fold = np.array(self.f, dtype=np.float64)
